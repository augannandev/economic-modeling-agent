...
let exportEndpointAvailable = null; // null = unknown, true = available, false = unavailable

/**
 * Check if the export endpoint is available
 */
async function checkExportEndpointAvailability() {
  try {
...
async function exportEmulatorData() {
  ...
  try {
    ...
    // Check if export endpoint is available (only check once after a few attempts)
    if (exportEndpointAvailable === null && backupCount > 3) {
      exportEndpointAvailable = await checkExportEndpointAvailability();
      if (!exportEndpointAvailable) {
        console.log('ℹ️  Export endpoint not available (auth-only emulator). Using --export-on-exit for backups.');
      }
    }
    
    // Skip export attempts if endpoint is known to be unavailable
    if (exportEndpointAvailable === false) {
      return;
    }
    ...
    if (response.ok) {
      ...
      // Export endpoint not available (common with auth-only emulator)
    ...
    if (backupCount > 2 && exportEndpointAvailable !== false) {
