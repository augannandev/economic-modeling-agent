...
export function getPythonServiceUrl(): string {
  return getEnv('PYTHON_SERVICE_URL', 'http://localhost:8000');
}

/**
 * Get R service URL from environment
 */
export function getRServiceUrl(): string {
  return getEnv('R_SERVICE_URL', 'http://localhost:8001');
}
...
export function getDataDirectory(): string {
  return getEnv('DATA_DIRECTORY', './my-app/PseuodoIPD');
}

/**
 * Get plots directory path
 */
export function getPlotsDirectory(): string {
  return getEnv('PLOTS_DIRECTORY', './data/plots');
}

/**
 * Get SEER data path
 */
export function getSeerDataPath(): string | undefined {
  return getEnv('SEER_DATA_PATH');
} 